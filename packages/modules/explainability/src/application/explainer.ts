...
function generateSummary(
  ...
  if (decisionMode === "random_fallback") {
    return `Assigned to ${agentName} (random selection). No clear scoring winner found.`;
  }
  
  if (decisionMode === "override") {
    return `Manually assigned to ${agentName} by manager (override).`;
  }
  
  // Scored assignment
  const topReason = breakdown.primaryReasons[0];
  if (!topReason) {
    return `${agentName} recommended (score: ${score}/100).`;
  }
  
  return `${agentName} is the best match (score: ${score}/100). Primary reason: ${topReason.explanation}`;
}
